...
def clear_images_data():
    ...
    # 1. Truncate image and annotation related tables
    tables = [
        "annotations",
        "images",
        "reward_transactions",
    ...
    if existing_tables:
        ...
        print(f"Truncating image & annotation tables: {tables_str}...")
        cur.execute(f"TRUNCATE TABLE {tables_str} CASCADE;")
        print("Image & annotation tables truncated successfully!")
        
    # Reset image counts on users
    cur.execute("UPDATE users SET image_count = 0;")
    print("Reset all user image counts to 0.")
        
    cur.close()
    conn.close()

    # 2. Clear image files from uploads folder
    ...
    print("\n--- ALL IMAGES & VALIDATION DATA SUCCESSFULLY CLEARED TO ZERO ---")
